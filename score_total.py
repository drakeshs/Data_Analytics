...
if os.path.exists('score_total.txt'):
    os.remove('score_total.txt')
if os.path.exists('score_total1.txt'):
    os.remove('score_total1.txt')
...
#***********************************************************************************
#generating file with employee info included
#***********************************************************************************
...
# List of your files
file_names = ['score_total1.txt', 'score_total.txt']
...
# through the generator objects and 
# write out to the output file
with open('total.txt', 'w') as op_file:
    csv_writer = csv.writer(op_file, delimiter=',')
    for row in list(zip(*o_data)):
        csv_writer.writerow(row)
op_file.close()
...
if os.path.exists('sum_file.txt'):
    os.remove('sum_file.txt')
if os.path.exists('score_total.csv'):
    os.remove('score_total.csv')
if os.path.exists('score_total1.txt'):
    os.remove('score_total1.txt')
